...
def render_360_videos():
    with open('holodeck_params.txt', 'r') as f:
    ...
    def get_whd(scene):
        try:
            dims_str = np.array(eval(scene['raw_floor_plan'].split('|')[-1]))
        except:
            dims_str = np.array(eval(scene['raw_floor_plan'].split('|')[3].split('\n')[0]))
        w, d = np.max(dims_str, axis=0)
        h = scene['wall_height']
        return w, h, d
    ...
    for file in os.listdir(glb_directory):
        if file.endswith('.json'):
            scene_json = os.path.join(glb_directory, file)
            break

    if scene_json is None:
        raise FileNotFoundError(f"No .json file found in the directory: {glb_directory}")
